...
namespace UnityEngine.UI
{
    ...
    public class LoopVerticalScrollRect : LoopScrollRect
    {
        ...
        protected override bool UpdateItems(ref Bounds viewBounds, ref Bounds contentBounds)
        {
            ...
            if (viewBounds.min.y - threshold < contentBounds.min.y + m_ContentBottomPadding)
            {
                float size = NewItemAtEnd(), totalSize = size;
                while (size > 0 && viewBounds.min.y - threshold < contentBounds.min.y + m_ContentBottomPadding - totalSize)
                {
                    size = NewItemAtEnd();
                    totalSize += size;
                }
            ...
            if (viewBounds.max.y + threshold > contentBounds.max.y - m_ContentTopPadding)
            {
                float size = NewItemAtStart(), totalSize = size;
                while (size > 0 && viewBounds.max.y + threshold > contentBounds.max.y - m_ContentTopPadding + totalSize)
                {
                    size = NewItemAtStart();
                    totalSize += size;
                }
            ...
            var endChild = m_Content.GetChild(m_Content.childCount - 1).GetComponent<RectTransform>();
            var endChildHeight = GetSize(endChild, false);
            if (viewBounds.min.y - threshold > contentBounds.min.y + endChildHeight + m_ContentBottomPadding)
            {
                float size = DeleteItemAtEnd(), totalSize = size;
                while (size > 0 && viewBounds.min.y - threshold > contentBounds.min.y + endChildHeight + m_ContentBottomPadding + totalSize)
                {
            ...
            var startChild = m_Content.GetChild(0).GetComponent<RectTransform>();
            var startChildHeight = GetSize(startChild, false);
            if (viewBounds.max.y + threshold < contentBounds.max.y - startChildHeight - m_ContentTopPadding)
            {
                float size = DeleteItemAtStart(), totalSize = size;
                while (size > 0 && viewBounds.max.y + threshold < contentBounds.max.y - startChildHeight - m_ContentTopPadding - totalSize)
                {
