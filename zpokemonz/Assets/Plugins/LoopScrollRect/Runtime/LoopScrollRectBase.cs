...
namespace UnityEngine.UI
{
    ...
    public abstract class LoopScrollRectBase : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        public float thresholdFactor = 1;
        ...
        public void ScrollToCellWithinTime(int index, float time, Action onScrollEnd = null)
        {
            if (totalCount >= 0 && (index < 0 || index >= totalCount))
            {
                Debug.LogErrorFormat("invalid index {0}", index);
            ...
            StartCoroutine(ScrollToCellCoroutine(index, Mathf.Abs(dist) / time, onScrollEnd));
        }

        IEnumerator ScrollToCellCoroutine(int index, float speed, Action onScrollEnd = null)
        {
        ...
        protected float NewItemAtStart(bool includeSpacing = true)
        {
            ...
            threshold = Mathf.Max(threshold, size * thresholdFactor);
        ...
        public void MockUpdateScroll()
        {
            SetContentAnchoredPosition(new Vector2(0, 1));
        }

        public void MockUpdateScroll(Vector2 position)
        {
            SetContentAnchoredPosition(position);
        }
